        ...
        (request, resolveContext, callback) => {
          ...
          for (const name of alias) {
            if (innerRequest.startsWith(name)) {
              ...
              let newRequestPath = path.resolve(
                dynamicPath,
                innerRequest.substr(name.length + 1)
              )
              if (!hasExtension(newRequestPath)) {
                const newRequestPathsWithExtension =
                  getNewRequestPathsWithExtension(newRequestPath, extensions)
...
function hasExtension(filePath = '') {
  const extensionPattern = /\.[a-z]{2,3}$/i
  return extensionPattern.test(filePath)
}

function getNewRequestPathsWithExtension(newRequestPath, extensions) {
  return [
    ...extensions.map(ext => `${newRequestPath}.${ext}`),
    ...extensions.map(ext => `${newRequestPath + '/index'}.${ext}`)
  ]
}
